...
func valueAt(values []*ast.MappingValueNode, key string) (ast.Node, bool) {
	...
	for _, f := range values {
		if strings.ToLower(keyString(f)) == key {
			return f.Value, true
		}
	}
	return nil, false
...
// getObjectProperty returns the value in object with the given key, if any.
func getObjectProperty(object *model.ObjectConsExpression, key string) (model.Expression, bool) {
	for _, i := range object.Items {
		if k, ok := getStringValue(i.Key); ok && k == key {
			return i.Value, true
		}
	}
	return nil, false
