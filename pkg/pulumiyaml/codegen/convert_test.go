...
`,
		...
		{
			name:     "partial inputs for patch",
			token:    "snippet:index:Bucket",
			filename: "patch.yaml",
			// Bucket declares name/size/versioned/tags but a patch-style caller may only
			// touch one attribute; the importer must emit just that one without complaining
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			body, diags, err := ImportSnippet(
				t.Context(), tt.token, tt.filename, []byte(tt.input), snippetPackage{}, snippetPackageLoader{})
...
func TestImportSnippet_UnknownToken(t *testing.T) {
	...
	_, _, err := ImportSnippet(
		...
		[]byte("name: foo\n"), snippetPackage{}, snippetPackageLoader{})
	require.Error(t, err)
...
func TestImportSnippetAttributes(t *testing.T) {
	t.Parallel()

	// Each attribute value is its own independent YAML expression source; the importer types
	// plain scalars against the schema and passes interpolations / structured values through.
...
`,
	...
	out, diags, err := ImportSnippetAttributes(
		t.Context(), attrs, "snippet:index:Bucket", snippetPackage{}, snippetPackageLoader{})
	require.NoError(t, err)
	require.Falsef(t, diags.HasErrors(), "unexpected diagnostics: %v", diags)
	assert.Equal(t, expected, out)
}

func TestImportSnippetAttributes_Empty(t *testing.T) {
	t.Parallel()
	out, diags, err := ImportSnippetAttributes(
		t.Context(), nil, "snippet:index:Bucket", snippetPackage{}, snippetPackageLoader{})
	require.NoError(t, err)
	require.Falsef(t, diags.HasErrors(), "unexpected diagnostics: %v", diags)
	assert.Nil(t, out)
}
