...
`
	...
	err = pulumi.RunErr(func(ctx *pulumi.Context) error {
		...
		cwdOutput, ok := ectx.evaluateExpr(ast.MustInterpolate("${pulumi.cwd}"))
		assert.True(t, ok)
		assert.Equal(t, cwd, cwdOutput)

		projectOutput, ok := ectx.evaluateExpr(ast.MustInterpolate("${pulumi.project}"))
		assert.True(t, ok)
		assert.Equal(t, "projectFoo", projectOutput)

		stackOutput, ok := ectx.evaluateExpr(ast.MustInterpolate("${pulumi.stack}"))
		assert.True(t, ok)
...
func testVariableDiags(t *testing.T, template *ast.TemplateDecl, callback func(*runner)) syntax.Diagnostics {
	...
	err := pulumi.RunErr(func(ctx *pulumi.Context) error {
		...
		ectx := runner.newContext(nil)
		v, ok := ectx.evaluateExpr(ast.MustInterpolate("${resFinal.out}"))
		assert.True(t, ok)
