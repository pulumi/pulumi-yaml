...
type Package interface {
	...
	ResourceTypeHint(typeName ResourceTypeToken) ResourceTypeHint
	// Information on the argument to a function.
	FunctionTypeHint(typeName FunctionTypeToken) TypeHint
}

type TypeHint interface {
	// If the TypeHint represents a type with named fields, drill into field `name`.
...
type ResourceTypeHint interface {
	TypeHint

	// The input properties for a resource. This is a subset of `Fields`.
	InputProperties() map[string]TypeHint
}
...
func (p resourcePackage) FunctionTypeHint(typeName FunctionTypeToken) TypeHint {
	f, ok := p.GetFunction(typeName.String())
	if !ok {
		return nil
	}
	return typeHintFromType(f.Inputs)
}
...
func (r fieldTypeHint) Fields() map[string]TypeHint {
	m := map[string]TypeHint{}
...
func (e elementTypeHint) Fields() map[string]TypeHint {
	return nil
}
