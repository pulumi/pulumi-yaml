...
type Package interface {
	...
	ResourceTypeHint(typeName ResourceTypeToken) TypeHint
...
func (p resourcePackage) ResourceTypeHint(typeName ResourceTypeToken) TypeHint {
	if _, ok := p.resolveProvider(typeName.String()); ok {
		return fieldTypeHint{p.Package.Provider.Properties}
	}
	r, ok := p.GetResource(typeName.String())
	if !ok {
		return nil
	}
	return fieldTypeHint{r.Properties}
}
